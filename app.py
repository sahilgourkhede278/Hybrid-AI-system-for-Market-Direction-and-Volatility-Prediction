from st_keyup import st_keyupimport reimport uuidfrom streamlit_cookies_manager import EncryptedCookieManagerfrom zoneinfo import ZoneInfoimport osimport psycopg2from psycopg2 import IntegrityErrorimport hashlibfrom datetime import datetime, date, timedeltaimport numpy as npimport pandas as pdimport plotly.express as pximport plotly.graph_objects as goimport streamlit as stimport yfinance as yffrom sklearn.ensemble import RandomForestClassifierfrom sklearn.linear_model import LogisticRegressionfrom sklearn.metrics import accuracy_scorefrom sklearn.model_selection import train_test_splitfrom sklearn.neighbors import KNeighborsClassifierfrom streamlit_autorefresh import st_autorefresh
...
cookies = EncryptedCookieManager(prefix="ai_market_dashboard/",password="change-this-to-a-strong-password")
...
STOCKS = {"Reliance": "RELIANCE.NS","TCS": "TCS.NS","Infosys": "INFY.NS","HDFC Bank": "HDFCBANK.NS","ICICI Bank": "ICICIBANK.NS","State Bank of India": "SBIN.NS","Axis Bank": "AXISBANK.NS","Kotak Mahindra Bank": "KOTAKBANK.NS","IndusInd Bank": "INDUSINDBK.NS","Bajaj Finance": "BAJFINANCE.NS","Bajaj Finserv": "BAJAJFINSV.NS","HCL Tech": "HCLTECH.NS","Wipro": "WIPRO.NS","Tech Mahindra": "TECHM.NS","LTIMindtree": "LTIM.NS","Larsen & Toubro": "LT.NS","UltraTech Cement": "ULTRACEMCO.NS","Grasim": "GRASIM.NS","Asian Paints": "ASIANPAINT.NS","Titan": "TITAN.NS","Maruti Suzuki": "MARUTI.NS","Tata Motors": "TATAMOTORS.NS","Mahindra & Mahindra": "M&M.NS","Eicher Motors": "EICHERMOT.NS","Hero MotoCorp": "HEROMOTOCO.NS","Sun Pharma": "SUNPHARMA.NS","Dr Reddy's": "DRREDDY.NS","Cipla": "CIPLA.NS","Apollo Hospitals": "APOLLOHOSP.NS","Divi's Labs": "DIVISLAB.NS","Hindustan Unilever": "HINDUNILVR.NS","ITC": "ITC.NS","Nestle India": "NESTLEIND.NS","Britannia": "BRITANNIA.NS","Tata Consumer": "TATACONSUM.NS","Adani Enterprises": "ADANIENT.NS","Adani Ports": "ADANIPORTS.NS","NTPC": "NTPC.NS","Power Grid": "POWERGRID.NS","ONGC": "ONGC.NS","Coal India": "COALINDIA.NS","JSW Steel": "JSWSTEEL.NS","Tata Steel": "TATASTEEL.NS","Hindalco": "HINDALCO.NS","BPCL": "BPCL.NS","Shree Cement": "SHREECEM.NS","SBI Life": "SBILIFE.NS","HDFC Life": "HDFCLIFE.NS","ICICI Prudential Life": "ICICIPRULI.NS","UPL": "UPL.NS","Bharat Electronics": "BEL.NS","Trent": "TRENT.NS","Pidilite": "PIDILITIND.NS","DLF": "DLF.NS","Jio Financial Services": "JIOFIN.NS"}

PORTFOLIO_STOCKS = {k: v for k, v in STOCKS.items() if k != "NIFTY 50"}PRACTICE_STOCKS = {k: v for k, v in STOCKS.items() if k != "NIFTY 50"}
...
def get_connection():return psycopg2.connect(st.secrets["DATABASE_URL"],connect_timeout=10)

def init_db():conn = get_connection()cur = conn.cursor()
...
def check_password_strength(password):score = 0
...
def create_user(full_name, username, password):conn = get_connection()cur = conn.cursor()
...
def verify_user(username, password):conn = get_connection()cur = conn.cursor()
...
def get_user_by_id(user_id):conn = get_connection()cur = conn.cursor()
...
def create_user_session(user_id):token = str(uuid.uuid4())expiry = datetime.now() + timedelta(days=7)
...
def get_user_from_session(token):conn = get_connection()cur = conn.cursor()
...
def delete_user_session(token):if not token:return
...
def get_user_portfolio(user_id):conn = get_connection()query = """SELECT stock, ticker, qty, buying_date, buying_priceFROM portfolioWHERE user_id = %s"""df = pd.read_sql_query(query, conn, params=(user_id,))conn.close()return df

def add_or_update_portfolio(user_id, stock, ticker, qty, buying_date, buying_price):conn = get_connection()cur = conn.cursor()
...
def get_dummy_balance(user_id):conn = get_connection()cur = conn.cursor()
...
def update_dummy_balance(user_id, new_balance):conn = get_connection()cur = conn.cursor()
...
def get_dummy_portfolio(user_id):conn = get_connection()query = """SELECT stock,ROUND((SUM(price * qty) / SUM(qty))::numeric, 2) AS price,SUM(qty) AS qtyFROM dummy_portfolioWHERE user_id = %sGROUP BY stock"""df = pd.read_sql_query(query, conn, params=(user_id,))conn.close()return df

def add_dummy_stock(user_id, stock, price, qty):conn = get_connection()cur = conn.cursor()
...
def sell_dummy_stock(user_id, stock, price, qty_to_sell):conn = get_connection()cur = conn.cursor()
...
def get_networth_data(user_id):conn = get_connection()cur = conn.cursor()
...
def save_networth_data(user_id,cash, investments, property_val, vehicles, gold,home_loan, home_rate, home_years,car_loan, car_rate, car_years,education_loan, education_rate, education_years,credit_card, credit_rate, credit_years,other_loan, other_rate, other_years):conn = get_connection()cur = conn.cursor()
...
@st.cache_data(ttl=300, show_spinner=False)def fetch_history_data(ticker_symbol, start_date, end_date):data = yf.download(ticker_symbol,start=start_date,end=end_date,progress=False,auto_adjust=True,threads=True)if data is None or data.empty:return pd.DataFrame()
...
@st.cache_data(ttl=300, show_spinner=False)def fetch_latest_prices_batch(ticker_list):ticker_list = [t for t in ticker_list if t]if not ticker_list:return {}
...
@st.cache_data(ttl=86400, show_spinner=False)def is_valid_market_day(ticker_symbol, selected_date_str):try:selected_date = pd.to_datetime(selected_date_str).date()next_date = selected_date + timedelta(days=1)
...
@st.cache_data(ttl=5, show_spinner=False)def fetch_live_intraday_data(ticker_symbol):data = yf.download(ticker_symbol,period="1d",interval="1m",progress=False,auto_adjust=True,threads=True)
...
def is_market_open():now = datetime.now(ZoneInfo("Asia/Kolkata"))
...
def is_live_market_tradable(ticker_symbol):if not is_market_open():return False

data = fetch_live_intraday_data(ticker_symbol)
return not data.empty

def go_to_page(page_name):st.session_state.page = page_namest.query_params["page"] = page_namest.rerun()

def logout_user():token = cookies.get("session_token")
...
@st.cache_data(ttl=300, show_spinner=False)def fetch_stock_info_data(ticker_symbol):try:stock_obj = yf.Ticker(ticker_symbol)info = stock_obj.info
...
def format_large_number(value):if value == "N/A" or pd.isna(value):return "N/A"
...
def format_percentage(value):if value == "N/A" or pd.isna(value):return "N/A"
...
@st.cache_resourcedef initialize_database_once():init_db()
...
if query_page:st.session_state.page = query_page

cookie_token = cookies.get("session_token")

if cookie_token and not st.session_state.logged_in:saved_user = get_user_from_session(cookie_token)
...
if not st.session_state.logged_in:st.title("🔐 AI Market Dashboard Login")
...
current_user = st.session_state.current_useruser_id = current_user["id"]

top1, top2, top3 = st.columns([5, 2, 1])with top1:st.caption(f"Logged in as: {current_user['full_name']} (@{current_user['username']})")with top2:st.caption(f"User ID: {user_id}")with top3:if st.button("Logout"):logout_user()
